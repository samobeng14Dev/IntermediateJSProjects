...
formEl.addEventListener("submit", (e) => {
	...
	//check if value is not empty, <1 or > 9
	if (isNaN(value) || value <= 0 || value > 9) {
		resultEl.innerHTML = `<p class="result">${text[random]}</p>`;
	} else {
		// generate text based on the value entered
		let tempText = text.slice(0, value);
		// loop over the generated text to display the result
		tempText = tempText
			.map((item) => {
	...
	const paragraphCopied = resultEl.textContent;
	copyToClipboard(paragraphCopied);
});
...
const copyToClipboard = (text) => {
	navigator.clipboard.writeText(text).then(() => {
		const message = `Paragraph copied to clippboard`;
		showMessage(message);
	});
};
